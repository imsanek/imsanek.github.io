                    ...
                    for(var tick = 0; tick < ticketsJSON.length; tick++){
                        ...
                        $('.ticket-block:eq('+tick+')').click(function(){
                            ...
                            $('.full-content #form').append($('<span/>', {
                                ...
                                'type': 'text',
                                'class': 'full2-text',
                                'value': ticketsJSON[timeTarget].price
                            })).append($('<input/>', {
                                'type': 'submit',
                                'class': 'send-button',
                ...
                if(apiJSON){
                    for(var i = 0; i < apiJSON.dates.length; i++){
                        ...
                        for(var j = 0; j < apiJSON.dates[i].date.length; j++){
                                ...
                                if(x){
                                    if($(x).attr('class').split(' ').pop() == 'red'){
                                        x.classList.remove("red");
                                        x.classList.add("grad_red");

                                        x.setAttribute('name', ($(x).attr('name') + " " + apiJSON.dates[i].name));
                                        x.setAttribute('phone', ($(x).attr('phone') + " "  + apiJSON.dates[i].phone));
                                        x.setAttribute('email', ($(x).attr('email') + " "  + apiJSON.dates[i].email));
                                    }else {
                                        x.classList.add("red");
                    ...
                    $('.grad_red').hover(function(){
                        $('.name').text("Выезжает: " + $(this).attr('name').split(' ')[0] + " | Заезжает: " + $(this).attr('name').split(' ')[1]);
                        $('.phone').text("Выезжает: " + $(this).attr('phone').split(' ')[0] + " | Заезжает: " + $(this).attr('phone').split(' ')[1]);
                        $('.email').text($(this).attr('email').split(' ')[0] + " | " + $(this).attr('email').split(' ')[1]);
                    });
